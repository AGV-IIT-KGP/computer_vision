...
if __name__=="__main__":
	...
	#print k
	final = thresholding(gray,200)  #binarize the image with manual thresholding of 200
	plt.imshow(final, cmap = cm.gray)
	plt.show()

	final = kernel_iteration(gray,180) #iterate kernel-wise and make the whole kernel black or white 
	plt.imshow(final, cmap = cm.gray)
	plt.show()
	
	final = hough_transform(final, 5)  #hough transform requires binary image, second parameter is for number of lines 
	plt.imshow(final, cmap = cm.gray)
	plt.show()