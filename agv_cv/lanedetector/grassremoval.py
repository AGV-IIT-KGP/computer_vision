...
def grassremoval(img):
    gray = rgb2gray(img)
    gray = normalize(gray)  
    gray = gaussian_blur(gray)
    final = thresholding(gray,200) #second parameter is the threshold for making image binary
    final = kernel_iteration(gray,180) #second parameter is the threshold to convert image to black and white kernels. If sum of intensities
                                        #of all kernels is <9*threshold, it is converted to black
    final = hough_transform(final, 5) #second parameter is the number of lines to be displayed by hough.
    return final